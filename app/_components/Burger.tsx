...
const Burger = ({ navOpen, toggleNav }: BurgerProps) => {
  return (
    ...
    >
      <span
        className={clsx(
          "block h-[2px] w-6 rounded-full bg-[var(--text-primary)] transition-all duration-300 ease-in-out origin-center",
          navOpen && "translate-y-[8px] rotate-45"
        )}
      />
      <span
        className={clsx(
          "block h-[2px] rounded-full bg-[var(--text-primary)] transition-all duration-300 ease-in-out origin-center",
