...
import { CONSENT_DEFAULT_SCRIPT } from "@/constants/consentConfig";
...
export default function RootLayout({
  ...
  return (
    <html lang="sr-Latn-RS" suppressHydrationWarning>
      <head>
        {/* Google Consent Mode v2, set to all-denied BEFORE gtag can load (and
            updated to the saved choice for a returning visitor). Must come first
            so consent state exists before any tracker. */}
        <script
          id="consent-default"
        ...
        {/* Shortlinks append ?eid=<id> - the event_id the server already sent
            ...
            strip it from the URL before either of them reads location. It runs
            unconditionally - it only reads and cleans the URL, storing nothing. */}
        <script
          id="eid-capture"
      ...
      >
        ...
        <ConsentProvider>
          ...
          <CookieConsent />
          {/* Env decision stays here on the server so the non-public
              META_PIXEL_ID fallback still resolves; consent gating happens
              inside ConsentedAnalytics on the client. */}
          <ConsentedAnalytics
